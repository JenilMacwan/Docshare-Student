...
import com.example.student.databinding.ActivitySem3Binding;
...
public class Sem3 extends AppCompatActivity {
    ...
    private void fetchDocuments(String semester) {
        Log.d("SEM3_DEBUG", "Fetching documents for semester: " + semester);
        documentList.clear();

        List<Document> adapterDocumentList = adapter.getdocumentList();
        adapterDocumentList.clear();

        String[] subjectSubcollectionNames = {"asdfghjk","pic", "oop", "Sub","DC","Discrete Mathematics","Data Structures And Algorithms","Digital Electronics","IT Workshop","Database Management Systems","Notice","University Result","Mid Sem Result","Exam Timetable"}; // Ensure this is correct
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                Log.d("SEM3_DEBUG", "Firestore Document Data - Subject: " + subjectName + ", Name: " + name + ", URL: " + url);

                                if (name != null && url != null) {
                                    // Pass semester and subjectName to the Document constructor
                                    Document doc = new Document(name, url, subjectName, semester);
                                    documentList.add(doc);
                                    Log.d("SEM3_DEBUG", "Document object added: " + doc.getName() + ", Semester: " + doc.getSemester() + ", Subject: " + doc.getSubjectName());
                                } else {
                            ...
                            if (subcollectionQueriesCompleted[0] == totalSubcollections) {
                                Log.d("SEM3_DEBUG", "All subcollection queries COMPLETED. Calling notifyDataSetChanged NOW."); // Log final list size
