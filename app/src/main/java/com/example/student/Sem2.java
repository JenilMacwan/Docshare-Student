...
import com.example.student.databinding.ActivitySem2Binding;
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                Log.d("SEM2_DEBUG", "Firestore Document Data - Subject: " + subjectName + ", Name: " + name + ", URL: " + url);

                                if (name != null && url != null) {
                                    // Pass semester and subjectName to the Document constructor
                                    Document doc = new Document(name, url, subjectName, semester);
                                    documentList.add(doc);
                                    Log.d("SEM2_DEBUG", "Document object added: " + doc.getName() + ", Semester: " + doc.getSemester() + ", Subject: " + doc.getSubjectName());
                                } else {
                            ...
                            if (subcollectionQueriesCompleted[0] == totalSubcollections) {
                                Log.d("SEM2_DEBUG", "All subcollection queries COMPLETED. Calling notifyDataSetChanged NOW."); // Log final list size
