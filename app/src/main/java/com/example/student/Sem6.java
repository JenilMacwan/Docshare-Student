...
import com.example.student.databinding.ActivitySem6Binding;
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                Log.d("SEM6_DEBUG", "Firestore Document Data - Subject: " + subjectName + ", Name: " + name + ", URL: " + url);

                                if (name != null && url != null) {
                                    // Pass semester and subjectName to the Document constructor
                                    Document doc = new Document(name, url, subjectName, semester);
                                    documentList.add(doc);
                                    Log.d("SEM6_DEBUG", "Document object added: " + doc.getName() + ", Semester: " + doc.getSemester() + ", Subject: " + doc.getSubjectName());
                                } else {
                                    Log.w("SEM6_DEBUG", "Document with null name or url found in " + subjectName);
                                }
                            }
                            subcollectionQueriesCompleted[0]++;
                            if (subcollectionQueriesCompleted[0] == totalSubcollections) {
                                Log.d("SEM6_DEBUG", "All subcollection queries COMPLETED. Calling notifyDataSetChanged NOW."); // Log final list size
